...
public final class CameraManager {
    ...
    public CameraManager(Context context) {
        ...
        int size = (int) (context.getResources().getDisplayMetrics().widthPixels * 0.8f);
        minFrameWidth = size;
        minFrameHeight = size;
        maxFrameWidth = size;
        maxFrameHeight = size;
    ...
    public synchronized void openDriver(SurfaceHolder holder) throws IOException {
        ...
        try {
            ...
            // Driver failed
            Log.w(QrCodeUtils.TAG, "Camera rejected parameters. Setting only minimal safe-mode parameters");
            Log.i(QrCodeUtils.TAG, "Resetting to saved camera params: " + parametersFlattened);
            // Reset:
            if (parametersFlattened != null) {
                ...
                try {
                    ...
                    // Well, darn. Give up
                    Log.w(QrCodeUtils.TAG, "Camera rejected even safe-mode parameters! No configuration");
                }
            }
        }
    ...
    public synchronized Rect getFramingRect() {
        if (framingRect == null) {
            ...
            Log.d(QrCodeUtils.TAG, "Calculated framing rect: " + framingRect);
        }
        return framingRect;
    }
    ...
    public synchronized void setManualFramingRect(int width, int height) {
        if (initialized) {
            ...
            int topOffset = (screenResolution.y - height) / 2;
            framingRect = new Rect(leftOffset, topOffset, leftOffset + width, topOffset + height);
            Log.d(QrCodeUtils.TAG, "Calculated manual framing rect: " + framingRect);
            framingRectInPreview = null;
        } else {
