        ...
        for (Integer minVersion : implementations.keySet()) {
            if (Build.VERSION.SDK_INT >= minVersion) {
                ...
                try {
                    ...
                    Log.i(QrCodeUtils.TAG, "Using implementation " + clazz + " of " + managedInterface + " for SDK " + minVersion);
                    return clazz.getConstructor().newInstance();
                } catch (ClassNotFoundException cnfe) {
                    Log.w(QrCodeUtils.TAG, cnfe);
                } catch (IllegalAccessException iae) {
                    Log.w(QrCodeUtils.TAG, iae);
                } catch (InstantiationException ie) {
                    Log.w(QrCodeUtils.TAG, ie);
                } catch (NoSuchMethodException nsme) {
                    Log.w(QrCodeUtils.TAG, nsme);
                } catch (InvocationTargetException ite) {
                    Log.w(QrCodeUtils.TAG, ite);
                }
            }
        }
        Log.i(QrCodeUtils.TAG, "Using default implementation " + defaultImplementation.getClass() + " of " + managedInterface);
