...
public final class CaptureActivityHandler extends Handler {
    ...
    public void handleMessage(Message message) {
        ...
        if (what == R.id.qrcode_restart_preview) {
            restartPreviewAndDecode();
        } else if (what == R.id.qrcode_decode_succeeded) {
            state = State.SUCCESS;
            Bundle bundle = message.getData();
            Bitmap barcode = bundle == null ? null : (Bitmap) bundle.getParcelable(DecodeThread.BARCODE_BITMAP);
            activity.handleDecode((Result) message.obj, barcode);
        } else if (what == R.id.qrcode_decode_failed) {
            state = State.PREVIEW;
            cameraManager.requestPreviewFrame(decodeThread.getHandler(), R.id.qrcode_decode);
        } else if (what == R.id.qrcode_return_scan_result) {
            activity.setResult(Activity.RESULT_OK, (Intent) message.obj);
            activity.finish();
        } else if (what == R.id.qrcode_launch_product_query) {
            Log.d(QrCodeUtils.TAG, "Got product query message");
            ...
            if (resolveInfo.activityInfo != null) {
                ...
                Log.d(QrCodeUtils.TAG, "Using browser in package " + browserPackageName);
            }
            ...
            try {
                ...
                Log.w(QrCodeUtils.TAG, "Can't find anything to handle VIEW of URI " + url);
            }
        }
    }
