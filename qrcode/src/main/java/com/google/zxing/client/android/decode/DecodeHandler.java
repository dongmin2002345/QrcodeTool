...
final class DecodeHandler extends Handler {
    ...
    public void handleMessage(Message message) {
        ...
        if (what == R.id.qrcode_decode) {
            decode((byte[]) message.obj, message.arg1, message.arg2);
        } else if (what == R.id.qrcode_quit) {
    ...
    private void decode(byte[] data, int width, int height) {
        ...
        if (rawResult != null) {
            ...
            Log.d(QrCodeUtils.TAG, "Found barcode in " + (end - start) + " ms");
            if (handler != null) {
                Message message = Message.obtain(handler, R.id.qrcode_decode_succeeded, rawResult);
            ...
            if (handler != null) {
                Message message = Message.obtain(handler, R.id.qrcode_decode_failed);
                message.sendToTarget();
            }
        }
    }
